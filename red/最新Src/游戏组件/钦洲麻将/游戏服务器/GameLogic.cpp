...
BYTE CGameLogic::AnalyseChiHuCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE cbCurrentCard, WORD wChiHuRight, tagChiHuResult & ChiHuResult)
{
	...
	//test
	//if (status==true) wChiHuKind|=CHR_QIANG_GANG;
	if (IsQingYiSe(cbCardIndexTemp,WeaveItem,cbWeaveCount)==true) wChiHuKind|=CHK_QING_YI_SE;
	if(IsQiXiaoDui(cbCardIndexTemp,WeaveItem,cbWeaveCount)==true)  wChiHuKind|=CHK_QI_XIAO_DUI;
	//if(IsOneColQiXiaoDui(cbCardIndexTemp,WeaveItem,cbWeaveCount)==true) wChiHuKind=CHK_ONE_XIAO_DUI;
	//if(ThirteenOne(cbCardIndexTemp,WeaveItem,cbWeaveCount)==true)  wChiHuKind|=CHK_THIRTEEN_ONE;
	...
	if (AnalyseItemArray.GetCount()>0)
	{
		...
		for (INT_PTR i=0;i<AnalyseItemArray.GetCount();i++)
		{
			...
			//if ((bLianCard==true)&&(bPengCard==false)) wChiHuKind=CHK_PING_HU;
	...
	//结果判断
	if(wChiHuKind==CHK_QING_YI_SE)wChiHuKind=CHK_NULL;
...
//清一色七小对
//bool CGameLogic::IsOneColQiXiaoDui(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount)
//{
//	if(!(IsOneColQiXiaoDui(cbCardIndex,WeaveItem,cbWeaveCount)&&IsQingYiSe(cbCardIndex,WeaveItem,cbWeaveCount)))return false;
//
//}
...
// 十三吆
//test
//bool CGameLogic::ThirteenOne(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount)
//{
//	 if (cbWeaveCount!=0) return false;
//	int  temp=39;
//	 BYTE ThirteenOne[]={0,8,9,17,18,26,27,28,29,30,31,32,33,};
//	 for (BYTE i=0;i<MAX_INDEX;i++)
//	 {
//		 for(BYTE j=0;j<13;j++)
//		 {
//			 if(!cbCardIndex[ThirteenOne[j]])return false;
//		 }
//		 for(BYTE j=0;j<13;j++)
//		 {
//			 if(cbCardIndex[ThirteenOne[j]]==2)
//			 {
//				 temp=j;
//				 break;
//			 }
//		 }
//		 if(temp>38)return false;
//		
//	 }
//
//	 return true;
