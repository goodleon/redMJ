...
{	
	...
	case GO_NORMAL_FINISH: 
		{	
			...
			for (WORD i=0;i<GAME_PLAYER;i++)
			{
				...
				if(m_cbCardStatus[i] != NO_CARD)
				{
					...
					if(GameEnd.lGameGold[i] > 0)
					{
						...
						GameEnd.lGameGold[i] -= (GameEnd.lGameGold[i] >= 100L) ? GameEnd.lGameGold[i] / 10L : 0L;
			...
			//游戏税收
			GameEnd.lTax += (m_lBankerScore >= 100L) ? m_lBankerScore / 10L : 0L;
			GameEnd.lGameGold[m_nBankerStation]   = m_lBankerScore - ((m_lBankerScore >= 100L) ? m_lBankerScore / 10L : 0L);
