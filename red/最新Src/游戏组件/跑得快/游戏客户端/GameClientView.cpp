...
CGameClientView::CGameClientView(void) : CGameFrameView(true,24)
{
	...
	m_ImageLogo.SetLoadInfo(IDB_LOGO,hInstance);
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//m_Timer.SetBitmap(IDB_TIMERBACK,IDB_TIMERBACK);
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	DeferWindowPos(hDwp,m_btStart,NULL,(nWidth-rcButton.Width())/2,(nHeight-rcButton.Height())/2+100,0,0,uFlags);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
  ...
  CBitmap* pOldBmp=Mem.SelectObject(&Bmp);
	...
	m_ImageLogo.AlphaDrawImage(&Mem,10,10,RGB(255,0,255));
...
1：outbottom:
	...
	int BaseStartPoint[][4]={
		...
		{BASEOFFESTX,(nHeight-m_szPisa.cy)/2,1,2},//left
		{(nWidth-m_szPisa.cx)/2,nHeight-m_szPisa.cy-BASEOFFESTY,0,2},//bottom
		{nWidth-m_szPisa.cx-BASEOFFESTX,(nHeight-m_szPisa.cy)/2,1,2}//right
	};
	...
	int OutStartPoint[][4]={
		...
		{BASEOFFESTX+m_szPisa.cx+OUTOFFEST,(nHeight-m_szPisa.cy)/2-BANKSIZE,0,0},//left
		{(nWidth-m_szPisa.cx)/2,nWidth-m_szPisa.cy*2-BASEOFFESTY-20-OUTOFFEST-BTNOFFEST,0,2},//bottom
		{nWidth-m_szPisa.cx*2-BASEOFFESTX-OUTOFFEST,(nHeight-m_szPisa.cy)/2-BANKSIZE,0,1}//right
	};
	...
	for(int i=0;i<GAME_PLAYER;i++)
	{
		...
		if (m_bOutPass[i])
		{
			...
			m_ImagePass.BitBlt(Mem,iStartPointX,iStartPointY);
		}
		else
		{
			//绘画已出的牌
	...
	if (m_bReady)
	{
		switch(m_nFocusIndex)
		{
			...
			case 3://右方
			{
				...
				if (m_bDeasilOrder)
					m_ImageFocus.AlphaDrawImage(&Mem,(nWidth-m_ImageFocus.GetWidth())/2+nOffWidth[m_nFocusIndex],(nHeight-m_ImageFocus.GetHeight())/2+nOffHeight[m_nFocusIndex],RGB(255,0,255));
				else
					m_ImageFocus.AlphaDrawImage(&Mem,(nWidth-m_ImageFocus.GetWidth())/2+nOffWidth1[m_nFocusIndex],(nHeight-m_ImageFocus.GetHeight())/2+nOffHeight1[m_nFocusIndex],RGB(255,0,255));
			}
				break;
		}
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		const tagUserData *pUserData=GetUserInfo(index);
						...
						//	top					left				bottom			right
		int nOffWidth[]=	{nWidth/2-PISAOFFEST*7-PISAOFFEST/2-100,
												150,					nWidth/2+PISAOFFEST*6+PISAOFFEST/2+m_szPisa.cx+20,
																					nWidth-150};
		int nOffHeight[]=	{150,				nHeight/2+PISAOFFEST*6+PISAOFFEST/2+m_szPisa.cy/2+20,
																	nHeight-180,
																					nHeight/2-PISAOFFEST*6-PISAOFFEST/2-m_szPisa.cy/2-40};
						//	top					right				bottom			left
		...
		int nOffHeight1[]=	{150,				nHeight/2-PISAOFFEST*6-PISAOFFEST/2-m_szPisa.cy/2-40,
																	nHeight-180,	nHeight/2+PISAOFFEST*6+PISAOFFEST/2+m_szPisa.cy/2+20};
		//绘画用户
		if (pUserData!=NULL)
		{
			//用户名字
			...
			if (m_bDeasilOrder)
			{	
				m_ImageHeadBg.BitBlt(Mem.m_hDC,nOffWidth[index]-36-2,nOffHeight[index]-2);
