...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	//按钮位置
	m_btPlacement.MoveWindow(nWidth-btRect.right-(nWidth-boardSize.cx)/2-150,nHeight-3*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btSavePlacement.MoveWindow(nWidth-btRect.right-(nWidth-boardSize.cx)/2-150,nHeight-2*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btLoadPlacement.MoveWindow(nWidth-btRect.right-(nWidth-boardSize.cx)/2-150,nHeight-btRect.bottom-80,btRect.right,btRect.bottom);
	m_btResetGame.MoveWindow(nWidth-btRect.right-(nWidth-boardSize.cx)/2-150,nHeight-3*btRect.bottom-80,btRect.right,btRect.bottom);

	m_btLoadHistory.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-4*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btPrevStep.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-3*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btNextStep.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-2*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btTurn.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-btRect.bottom-80,btRect.right,btRect.bottom);

	m_btLose.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-3*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btGiveUp.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-2*btRect.bottom-80,btRect.right,btRect.bottom);
	m_btPeace.MoveWindow(nWidth-2*btRect.right-(nWidth-boardSize.cx)/2-155,nHeight-btRect.bottom-80,btRect.right,btRect.bottom);
	...
	m_ChessBoard.SetExcursionY((nHeight-boardSize.cy)/2+58) ;
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pUserData!=NULL)
		{
			...
			if(i==0)
			{
				DrawUserFace(pDC,pUserData->wFaceID,400,50,pUserData->cbUserStatus==US_OFFLINE);
				pDC->TextOut(440,55,szBuffer,lstrlen(szBuffer));
			}
			else if(i==1)
			{
				DrawUserFace(pDC,pUserData->wFaceID,43,195,pUserData->cbUserStatus==US_OFFLINE);
				pDC->TextOut(83,205,szBuffer,lstrlen(szBuffer));
			}else if(i==3)
			{
				DrawUserFace(pDC,pUserData->wFaceID,400,195,pUserData->cbUserStatus==US_OFFLINE);
				pDC->TextOut(440,205,szBuffer,lstrlen(szBuffer));
			}
