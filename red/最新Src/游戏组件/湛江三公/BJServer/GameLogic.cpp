...
BYTE CGameLogic::GetCardKind(const BYTE bCardList[], const BYTE bCardCount)
{
	...
	//是否333超级三公
	if(IsSanZhangSan(bCardList,bCardCount)) return SanZhangSan;
	//是否三张一样公牌
	if(IsSanZhangGong(bCardList,bCardCount)) return SanZhangGong;
	//是否对子公牌
	if(IsSanGongDui(bCardList,bCardCount)) return SanGongDui;
	//是否JQK公牌
	if(IsSanGong(bCardList,bCardCount)) return SanGong;
	return SHAN; //返回为散牌
							 ...
							 const BYTE bIdleCard[],  const BYTE bIdleCardCount,BYTE &cbWinnerCardType)
{
	...
	//庄家赢
	if (bFirstKind>bLastKind) return 1;

	//闲家赢
	if (bFirstKind<bLastKind) return 2;
	...
	case SHAN: 
		{
			...
			bFirstCard = (GetCardValoe(bBankerCard[0])+GetCardValoe(bBankerCard[1])+GetCardValoe(bBankerCard[2]))%10;
			bLastCard = (GetCardValoe(bIdleCard[0])+GetCardValoe(bIdleCard[1])+GetCardValoe(bIdleCard[2]))%10;

			int ret = CompareSanCard(bBankerCard,bIdleCard,3);
			if (ret == 0 || ret == 1)
			{
				if (bFirstCard >= 8)
				{
					cbWinnerCardType = SHAN8;
					return 1;
				}
				else
				{
	...
	case SanGong:
		{
			cbWinnerCardType = SanGong;
			return 1; //都为JQK庄赢		
		}
		//是否对子公牌
	case SanGongDui: 
		{
			cbWinnerCardType = SanGongDui;
			return CompareSanGongDui(bBankerCard,bIdleCard);
		}		 
		//是否三张一样公牌
	case SanZhangGong: 
		{
			cbWinnerCardType = SanZhangGong;
			return CompareOnlyOne(bBankerCard[0],bIdleCard[0]);
		}
		//是否333超级三公
	case SanZhangSan: 
		{
			cbWinnerCardType = SanZhangSan;
			return 1; //这种情况1副牌不可能发生，除非是多副牌，预留 庄赢
		}
	}
