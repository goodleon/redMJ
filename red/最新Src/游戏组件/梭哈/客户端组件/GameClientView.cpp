...
CGameClientView::CGameClientView(void) : CGameFrameView(true,24)
{
	...
	{
		m_ImageGlodTable.SetLoadInfo(IDB_GLOD_TABLE_800,hInstance);
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	//设置坐标
	//以自己为最下家
	m_ptFace[0].x=nWidth/2-m_nXFace;      //对家
	...
	m_ptFace[1].x=nWidth-m_nXBorder-m_nXFace-5;   //右家
	...
	m_ptTimer[2].y=nHeight-m_nYBorder-m_nYFace-10-m_nYTimer;
	m_ptReady[2].x=nWidth/2+72+m_nXTimer;
	m_ptReady[2].y=nHeight-m_nYBorder-m_nYFace-10-m_nYTimer/2;

	m_ptFace[3].x=m_nXBorder+5;                         //左家
	...
	DeferWindowPos(hDwp,m_btFollow,NULL,nWidth-m_nXBorder-90,nHeight-m_nYBorder-85,0,0,uFlags);
	DeferWindowPos(hDwp,m_btGiveUp,NULL,nWidth-m_nXBorder-50,nHeight-m_nYBorder-50,0,0,uFlags);
	DeferWindowPos(hDwp,m_btShowHand,NULL,nWidth-m_nXBorder-155,nHeight-m_nYBorder-71,0,0,uFlags);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	m_ImageTotal.BitBlt(pDC->m_hDC,nWidth/2-m_ImageTotal.GetWidth()/2,nHeight/2-m_ImageTotal.GetHeight()/2);
	//绘画用户
	WORD wUserTimer=0;
	TCHAR szBuffer[64];
	...
	if (m_lBasicGold!=0) 
	{
		...
		//DrawText(pDC->GetSafeHdc(),szBuffer,lstrlen(szBuffer),CRect(86,35,162,50),uDrawFormat);
		DrawText(pDC->GetSafeHdc(),szBuffer,lstrlen(szBuffer),CRect(86,57,162,72),uDrawFormat);
	}
