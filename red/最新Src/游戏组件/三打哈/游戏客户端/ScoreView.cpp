...
void CScoreView::OnPaint() 
{
	...
	if (m_lGameTax!=0)
	{
		...
		_snprintf(szBufferStr,sizeof(szBufferStr),"游戏税率：%ld",m_lGameTax);
		dc.DrawText(szBufferStr,lstrlen(szBufferStr),&rcDraw,DT_VCENTER|DT_CENTER|DT_END_ELLIPSIS);
	}
	...
	rcDraw.top=180;
	rcDraw.bottom=rcDraw.top+12;
	dc.DrawText(msg,lstrlen(msg),&rcDraw,DT_VCENTER|DT_END_ELLIPSIS);
...
void CScoreView::SetScoreViewInfo(tagScoreViewInfo & ScoreViewInfo)
{
	...
	strcpy(msg,ScoreViewInfo.msg);
	m_lGameTax=ScoreViewInfo.lGameTax;
