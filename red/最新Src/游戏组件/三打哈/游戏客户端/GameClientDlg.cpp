...
#define TIME_START_GAME					20								//开始时间
#define TIME_SEND_CONCEAL				60								//留底时间
#define TIME_CALL_SCORE					15								//叫分时间
...
#define WAV_START							  3
#define WAV_PASS							  5
#define WAV_DO_GOLD_5								  5000
#define WAV_DO_GOLD_10							  5001
#define WAV_DO_GOLD_15							  5002
#define WAV_DO_GOLD_20							  5003
#define WAV_DO_GOLD_25							  5004
#define WAV_DO_GOLD_30							  5005
#define WAV_DO_GOLD_35							  5006
#define WAV_DO_GOLD_40							  5007
#define WAV_DO_GOLD_45							  5008
#define WAV_DO_GOLD_50							  5009
#define WAV_DO_GOLD_55							  5010
#define WAV_DO_GOLD_60							  5011
#define WAV_DO_GOLD_65							  5012
#define WAV_DO_GOLD_70							  5013
#define WAV_DO_GOLD_75							  5014
#define WAV_DO_GOLD_80							  5015

#define WAV_zhuang_FK							  6000
#define WAV_zhuang_MH							  6001
#define WAV_zhuang_RT							  6002
#define WAV_zhuang_HT							  6003

#define WAV_quick							 8
#define WAV_win								 9
#define WAV_LOST							 10

#define WAV_send_END4						7003
#define WAV_send_END3						7002
#define WAV_send_END2						7001
#define WAV_send_END1						7000
#define WAV_send_END5						7004
#define WAV_send_END6						7005

#define WAV_send5							8004
#define WAV_send4							8003
#define WAV_send2							8001
#define WAV_send1							8000
...
BEGIN_MESSAGE_MAP(CGameClientDlg, CGameFrameDlg)
	...
	ON_MESSAGE(IDM_SURRENDER,OnSurrender)
//	ON_WM_LBUTTONDOWN()
ON_WM_LBUTTONDOWN()
END_MESSAGE_MAP()
...
CGameClientDlg::CGameClientDlg() : CGameFrameDlg(&m_GameClientView)
{
	...
	m_GameClientView.m_CallScore.m_bMustCall=false;
	...
	m_GameClientView.m_CallScore.m_cbCount=0;
	...
	case IDI_OUT_CARD:			//用户出牌
		{
			...
			//播放声音
			if ((nElapse<=10)&&(nElapse>3)&&(wChairID==GetMeChairID())&&(IsLookonMode()==false)) PlayGameSoundFromFip2(WAV_quick,GetMeChairID());
	...
	case IDI_LAND_SCORE:		//叫分
		{
			...
			if ((nElapse==0)||((m_bStustee==true)&&(nElapse<(TIME_CALL_SCORE))))
			{
				...
				if ((m_cbLandCount==0)&&(m_cbMainCount<7)&&(IsLookonMode()==false)&&((m_wBankerUser==GetMeChairID())||(m_wBankerUser==INVALID_CHAIR))) 
				{
					OnLandScore(80,80);
					return true;
				}
				if ((m_cbLandCount!=0)&&(IsLookonMode()==false)&&(wChairID==GetMeChairID())&&(m_cbMainCount<7))
				{
					OnLandScore(250,250);
					return true;
				}
				if ((m_cbLandCount!=0)&&(m_cbBankScore>=70)&&(m_cbMainCount>=7)&&(IsLookonMode()==false)&&(wChairID==GetMeChairID()))
				{
					OnLandScore(65,65);
					return true;
				}
	...
	case IDI_SEND_CONCEAL:		//发送底牌
		{
			if ((nElapse==0)||((m_bStustee==true)&&(nElapse<(TIME_SEND_CONCEAL))))
			{
				...
				if ((IsLookonMode()==false)&&(wChairID==GetMeChairID()))
				{
					...
					m_GameClientView.m_btSurrender.ShowWindow(SW_HIDE);
					m_GameClientView.m_btSurrender.EnableWindow(FALSE);
					//叫主牌
	...
	case GS_UG_FREE:	//空闲状态
		{
			...
			if (IsLookonMode()==false)
			{
				...
				SetWavScript("/GameApp/WavScript.asp?KindName=三打哈");
				SetReadyIfAutoStart(&m_GameClientView.m_btStart);
			}
	...
	case GS_UG_BACK:	//留底状态
		{
			...
			if (IsLookonMode()==false) 
			{
				...
				if (m_wCurrentUser==GetMeChairID())
				{
					...
					m_GameClientView.m_btSurrender.EnableWindow(TRUE);
					m_GameClientView.m_btSurrender.ShowWindow(SW_SHOW);
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	if (IsLookonMode()==false)
	{
		for(BYTE i=0;i<GAME_PLAYER;i++)
		{
			for(BYTE n=0;n<60;n++)
			{
				m_cbOutCardCount[i][n]=0;
				m_cbOutCardData[i][n][n]=0;
			}
		}
	}
	//底牌变量
	...
	//PlayGameSoundFromFip2(WAV_START,INVALID_CHAIR);
...
bool CGameClientDlg::OnSubCallCard(const void * pBuffer, WORD wDataSize)
{
	...
	BYTE cbBankScore=pCallCard->cbBankScore;

	//逻辑组件
	m_GameLogic.SetMainColor(cbCallColor);
	m_GameLogic.SetMainValue(7);
	m_GameClientView.SetBankerScore(cbBankScore);  //设置庄分
...
bool CGameClientDlg::OnSubSendConceal(const void * pBuffer, WORD wDataSize)
{
	...
	m_cbBankScore=pSendConceal->cbBankScore;

	//状态设置
	SetGameStatus(GS_UG_BACK);
	m_GameClientView.SetBankerUser(SwitchViewChairID(m_wBankerUser)); //设置庄家
	m_GameClientView.SetBankerScore(m_cbBankScore);   //设置庄分
...
bool CGameClientDlg::OnSubGamePlay(const void * pBuffer, WORD wDataSize)
{
	...
	m_cbBankScore=pGamePlay->cbBankScore;
	m_cbMainColor=pGamePlay->cbMainColor;

	 //游戏视图
	m_GameClientView.SetBankerScore(m_cbBankScore);   //庄家分
	m_GameClientView.SetCallCardInfo(SwitchViewChairID(m_wBankerUser),m_cbMainColor,1);  //花色
	...
	if ((IsLookonMode()==false)&&(m_wCurrentUser==GetMeChairID()))
	{
		...
		if(m_cbMainColor==0x30) PlayGameSoundFromFip2(WAV_zhuang_HT,INVALID_CHAIR);
		if(m_cbMainColor==0x20) PlayGameSoundFromFip2(WAV_zhuang_RT,INVALID_CHAIR);
		if(m_cbMainColor==0x10) PlayGameSoundFromFip2(WAV_zhuang_MH,INVALID_CHAIR);
		if(m_cbMainColor==0x00) PlayGameSoundFromFip2(WAV_zhuang_FK,INVALID_CHAIR);
	
		//m_GameClientView.m_btOutPrompt.ShowWindow(m_wFirstOutUser==GetMeChairID()?SW_HIDE:SW_SHOW);
	}
...
bool CGameClientDlg::OnSubCallScore(const void * pBuffer, WORD wDataSize)
{
	...
	m_GameClientView.SetLandScore(wViewChairID,pLandScore->bCurrentScore);
	m_cbLandCount=pLandScore->bLandCount;       //叫分次数

	BYTE bBossScore=pLandScore->bPrevScore;     //上次叫分

	BYTE bPreScore=pLandScore->bCurrentScore;   //当前叫分
	m_cbPriScore=bPreScore;

	m_GameClientView.m_CallScore.m_cbPriScore=bPreScore;
	
	m_GameClientView.SetBankerScore(bPreScore);   //设置庄分
	m_cbBankScore=bPreScore;
	
	if ((IsLookonMode()==false)&&(pLandScore->wCurrentUser==GetMeChairID()))
	{
		//ActiveGameFrame();
		m_GameClientView.m_CallScore.m_bMustCall=(m_cbBankScore>=70&&m_cbMainCount>=7&&m_cbBankScore!=250)?true:false;
		m_GameClientView.m_CallScore.ShowWindow(SW_SHOW);
		m_GameClientView.m_CallScore.m_cbCount=m_cbLandCount;

		if (bBossScore==250)
		{
			PlayGameSoundFromFip2(WAV_PASS,GetMeChairID());
		}
		else
		{
			if (bBossScore==80) PlayGameSoundFromFip2(WAV_DO_GOLD_80,INVALID_CHAIR);
			if (bBossScore==75) PlayGameSoundFromFip2(WAV_DO_GOLD_75,INVALID_CHAIR);
			if (bBossScore==70) PlayGameSoundFromFip2(WAV_DO_GOLD_70,INVALID_CHAIR);
			if (bBossScore==65) PlayGameSoundFromFip2(WAV_DO_GOLD_65,INVALID_CHAIR);
			if (bBossScore==60) PlayGameSoundFromFip2(WAV_DO_GOLD_60,INVALID_CHAIR);
			if (bBossScore==55) PlayGameSoundFromFip2(WAV_DO_GOLD_55,INVALID_CHAIR);
			if (bBossScore==50) PlayGameSoundFromFip2(WAV_DO_GOLD_50,INVALID_CHAIR);
			if (bBossScore==45) PlayGameSoundFromFip2(WAV_DO_GOLD_45,INVALID_CHAIR);
			if (bBossScore==40) PlayGameSoundFromFip2(WAV_DO_GOLD_40,INVALID_CHAIR);
			if (bBossScore==35) PlayGameSoundFromFip2(WAV_DO_GOLD_35,INVALID_CHAIR);
			if (bBossScore==30) PlayGameSoundFromFip2(WAV_DO_GOLD_30,INVALID_CHAIR);
			if (bBossScore==25) PlayGameSoundFromFip2(WAV_DO_GOLD_25,INVALID_CHAIR);
			if (bBossScore==20) PlayGameSoundFromFip2(WAV_DO_GOLD_20,INVALID_CHAIR);
			if (bBossScore==15) PlayGameSoundFromFip2(WAV_DO_GOLD_15,INVALID_CHAIR);
			if (bBossScore==10) PlayGameSoundFromFip2(WAV_DO_GOLD_10,INVALID_CHAIR);
		}
	}
...
bool CGameClientDlg::OnSubOutCard(const void * pBuffer, WORD wDataSize)
{
	...
	*/
	...
	if ((IsLookonMode()==true)||(wMeChairID!=wOutCardUser))
	{
		...
		if (wOutCardUser==m_wFirstOutUser)
		{
			for (WORD i=0;i<GAME_PLAYER;i++) 
			{
				...
				//m_GameClientView.m_btOutPrompt.ShowWindow(m_wFirstOutUser==m_wCurrentUser?SW_HIDE:SW_SHOW);
			}
		}
	...
	{
		m_bLastTurns++;
		m_bTurn=m_bLastTurns;

		//设置扑克
		for (WORD i=0;i<GAME_PLAYER;i++) 
		{
			//保存扑克
			m_cbOutCardCount[i][m_bLastTurns]=m_cbOutCardCount[i][0];
			CopyMemory(m_cbOutCardData[i][m_bLastTurns],m_cbOutCardData[i][0],sizeof(BYTE)*m_cbOutCardCount[i][0]);
...
bool CGameClientDlg::OnSubThrowResult(const void * pBuffer, WORD wDataSize)
{
	...
	*/
	...
	PlayGameSoundFromFip2(WAV_send5,GetMeChairID());
	//清理扑克
	for (WORD i=0;i<GAME_PLAYER;i++) 
	{
		//设置扑克
...
bool CGameClientDlg::OnSubTurnBalance(const void * pBuffer, WORD wDataSize)
{
	...
	m_cbBankScore=pTurnBalance->cbBankScore;
	m_cbMainColor=pTurnBalance->cbMainColor;
	m_wBankerUser=pTurnBalance->cbBankUser;

	m_GameClientView.SetBankerScore(m_cbBankScore);
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	strcpy(ScoreViewInfo.msg,pGameEnd->strEnd);
	...
	if (lMeScore>0L) PlayGameSoundFromFip2(WAV_win,GetMeChairID());//PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_WIN"));
	else if (lMeScore<0L) PlayGameSoundFromFip2(WAV_LOST,GetMeChairID());//PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_LOST"));
	else PlayGameSound(GetModuleHandle(NULL),TEXT("GAME_END"));
		
	//设置界面
	if (IsLookonMode()==false)
	{
		if(strncmp(pGameEnd->strEnd,"小倒",4)==0) PlayGameSoundFromFip2(WAV_send_END4,GetMeChairID());
		if(strncmp(pGameEnd->strEnd,"清光",4)==0) PlayGameSoundFromFip2(WAV_send_END5,GetMeChairID());
		if(strncmp(pGameEnd->strEnd,"小光",4)==0) PlayGameSoundFromFip2(WAV_send_END3,GetMeChairID());
		if(strncmp(pGameEnd->strEnd,"过庄",4)==0) PlayGameSoundFromFip2(WAV_send_END6,GetMeChairID());
		if(strncmp(pGameEnd->strEnd,"垮庄",4)==0) PlayGameSoundFromFip2(WAV_send_END1,GetMeChairID());
		if(strncmp(pGameEnd->strEnd,"大倒",4)==0) PlayGameSoundFromFip2(WAV_send_END2,GetMeChairID());
...
bool CGameClientDlg::VerdictOutCard()
{
	...
	if (cbShootCount>0L)
	{
		...
		if (m_wFirstOutUser==GetMeChairID())
		{
			...
			//if (cbCardType==CT_ERROR||cbCardType==CT_THROW_CARD) return false;  //去掉甩牌
...
bool CGameClientDlg::DispatchUserCard(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	//设置定时器
	SetTimer(IDI_DISPATCH_CARD,250,NULL);
	...
	case IDI_DISPATCH_CARD:		//派发扑克
		{
			...
			if (m_cbRemnantCardCount>0)
			{
				...
				BYTE cbMainValue=m_GameLogic.GetCardLogicColor2(m_cbDispatchCardData[m_cbDispatchCardCount]); 

				if(cbMainValue==COLOR_NT) m_cbMainCount++;    //获取常主数量
			...
			if (m_cbRemnantCardCount==0) 
			{
				...
				if ((IsLookonMode()==false)&&(m_cbFocusUser==GetMeChairID())) 
				{
					//ActiveGameFrame();
					//设置时间
					//m_GameClientView.m_CallScore.m_bMustCall=(m_cbMainCount>=7)?true:false;
...
LRESULT CGameClientDlg::OnOutCard(WPARAM wParam, LPARAM lParam)
{
	...
	//类型判断
	cbCardType=m_GameLogic.GetCardType(OutCard.cbCardData,OutCard.cbCardCount);
	//花色判断
	if(OutCard.cbCardCount==1) cbCardType=m_GameLogic.GetCardLogicColor2(OutCard.cbCardData[0]);
	...
	case CT_TRACKOR_4:
		{
			PlayGameSoundFromFip2(WAV_send4,INVALID_CHAIR);
			break;
		}
		//对牌
	case CT_SAME_2:
		{
			PlayGameSoundFromFip2(WAV_send2,INVALID_CHAIR);
			break;
		}
	case COLOR_NT:
		{
			if(m_wBankerUser==GetMeChairID()) PlayGameSoundFromFip2(WAV_send1,INVALID_CHAIR);
			break;
		}
	}
...
LRESULT CGameClientDlg::OnCallCard(WPARAM wParam, LPARAM lParam)
{
	...
	CallCard.cbBankScore=m_cbBankScore;
...
LRESULT CGameClientDlg::OnSendConceal(WPARAM wParam, LPARAM lParam)
{
	...
	ConcealCard.cbBankScore=m_cbBankScore;
...
LRESULT CGameClientDlg::OnLastTurnCard(WPARAM wParam, LPARAM lParam)
{
	if (m_bLastTurn==false)
	{
		...
		{
			WORD wViewChairID=SwitchViewChairID(i);
			m_GameClientView.m_UserCardControl[wViewChairID].SetCardData(m_cbOutCardData[i][m_bTurn],m_cbOutCardCount[i][m_bTurn]);
		}
		//环境设置
...
//投降消息
LRESULT CGameClientDlg::OnSurrender(WPARAM wParam, LPARAM lParam)
{
	CMD_C_SURRENDER Surrender;
	Surrender.cbBankerUser=m_wBankerUser;

	//发送数据
	SendData(SUB_C_SURRENDER,&Surrender,sizeof(Surrender));
	return 0;
}
...
//游戏帮助
void CGameClientDlg::OnGameOptionHelp()
{
	m_GameClientView.SetHelp(true);
	return;
}
