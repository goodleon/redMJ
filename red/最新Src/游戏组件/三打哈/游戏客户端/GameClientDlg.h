...
protected:
	...
	BYTE							m_cbBankScore;						//庄家分数
	BYTE							m_cbMainCount;						//主牌数量
...
protected:
	...
	BYTE							m_bLastTurns;						//上轮牌标志
	BYTE							m_bTurn;						    //查看上轮牌点击次数
	WORD							m_wFirstOutUser;					//出牌用户
	BYTE							m_cbOutCardCount[4][MAX_COUNT];				//出牌数目
	BYTE							m_cbOutCardData[4][MAX_COUNT][MAX_COUNT];	//出牌列表
...
protected:
	...
	//投降
	LRESULT OnSurrender(WPARAM wParam, LPARAM lParam);
