	...
	case CT_TRACKOR_4:	//拖拉机型
		{
			...
			{
				//用户索引
				//WORD wUserIndex=(wFirstIndex+i+1)%GAME_PLAYER;(顺时针)
...
BYTE CGameLogic::GetCardLogicColor(BYTE cbCardData)
{
	...
	//花色判断
	if (cbCardColor==COLOR_NT) return COLOR_NT;       //王
	if (cbCardColor==m_cbMainColor) return COLOR_NT;  //主牌色

	//数值判断
	if (cbCardValue==m_cbNTValue) return COLOR_NT;     //2
	if (cbCardValue==m_cbMainValue) return COLOR_NT;   //7
