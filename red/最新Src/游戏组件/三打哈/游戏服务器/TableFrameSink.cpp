...
#if 1
#define MsgBox(s) ::MessageBox(0,s,0,0)
#else
#define MsgBox(s)
#endif
...
bool __cdecl CTableFrameSink::OnEventGameStart()
{
	...
	if(m_wBankerUser!=INVALID_CHAIR) m_wBankerUser=m_wBankerUser;
	...
	case GER_NORMAL:		//常规结束
		{
			...
			//统计变量
			LONG lBankerScore=0L;
			
			if(m_bIsTX)  //是否投降
			{
				//积分统计
				if (m_bLandScore>=55)   //跨庄
				{
					lBankerScore=-1L;
					strcpy(msg,"垮庄");
				}
			...
			else
			{
				...
				//扣底倍数
				if ((wChairID!=m_wBankerUser)&&(m_GameLogic.GetCardLogicColor(m_cbOutCardData[wChairID],m_cbOutCardCount[wChairID])==64))
				{
					//类型判断
				...
				if ((wWholeScore<30)&&(wWholeScore!=0)) 
				{
					lBankerScore=4L;
					strcpy(msg,"小光");
				}
				if ((wWholeScore<m_bLandScore)&&(m_bLandScore>30)&&(wWholeScore!=0)&&(wWholeScore>=30))
				{
					lBankerScore=2L;
					strcpy(msg,"过庄");
				}
				if ((wWholeScore>=m_bLandScore)&&(wWholeScore<(m_bLandScore+40)))  //跨庄
				{
					lBankerScore=-1L;
					strcpy(msg,"垮庄");
				}
				if ((wWholeScore>=m_bLandScore+40)&&(wWholeScore<(m_bLandScore+70)))  //小倒
				{
					lBankerScore=-2L;
					strcpy(msg,"小倒");
				}
				if (wWholeScore>=(m_bLandScore+70))  //大倒
				{
					lBankerScore=-3L;
					strcpy(msg,"大倒");
				}
			...
			//玩家得分
			GameEnd.lScore[m_wBankerUser]=3*m_pGameServiceOption->lCellScore*lBankerScore;
			GameEnd.lScore[(m_wBankerUser+2)%m_wPlayerCount]=-(m_pGameServiceOption->lCellScore*lBankerScore);
			GameEnd.lScore[(m_wBankerUser+1)%m_wPlayerCount]=-(m_pGameServiceOption->lCellScore*lBankerScore);
			GameEnd.lScore[(m_wBankerUser+3)%m_wPlayerCount]=-(m_pGameServiceOption->lCellScore*lBankerScore);
			
			/*GameEnd.lScore[wChairID]=lBankerScore*3*m_pGameServiceOption->lCellScore;
			GameEnd.lScore[(wChairID+2)%m_wPlayerCount]=-lBankerScore*m_pGameServiceOption->lCellScore;
			GameEnd.lScore[(wChairID+1)%m_wPlayerCount]=-lBankerScore*m_pGameServiceOption->lCellScore;
			GameEnd.lScore[(wChairID+3)%m_wPlayerCount]=-lBankerScore*m_pGameServiceOption->lCellScore;
			*/

			strcpy(GameEnd.strEnd,msg); //结束信息
			...
			if (m_pGameServiceOption->wServerType&GAME_GENRE_GOLD)
			{
				for (WORD i=0;i<m_wPlayerCount;i++)
				{
					...
					{
						GameEnd.lGameTax=GameEnd.lScore[i]*5/100;
						GameEnd.lScore[i]-=GameEnd.lGameTax;
						n++;
					}
				}
			}
			...
			//庄家切换
			if (GameEnd.lScore[m_wBankerUser]>0L) m_wBankerUser=m_wBankerUser;  //庄家过庄 继续当庄
			else
			{
				m_wBankerUser=(m_wBankerUser+3)%m_wPlayerCount;  //垮庄
				m_wFirstUser=m_wBankerUser;
			}
			
			if(n!=0) GameEnd.lGameTax=n*GameEnd.lGameTax;
...
bool __cdecl CTableFrameSink::OnGameMessage(WORD wSubCmdID, const void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_C_CALL_CARD:		//用户叫牌
		{
			...
			return OnUserCallCard(pUserData->wChairID,pCallCard->cbCallColor,pCallCard->cbBankScore);
		}
	...
	case SUB_C_SURRENDER:   //用户投降
		{
			//效验数据
			ASSERT(wDataSize==sizeof(CMD_C_SURRENDER));
			if (wDataSize!=sizeof(CMD_C_SURRENDER)) return false;
			...
			//消息处理
			CMD_C_SURRENDER * Surrender=(CMD_C_SURRENDER *)pDataBuffer;
			return OnUserSurrender(Surrender->cbBankerUser);
		}
	}
...
//用户叫牌
bool CTableFrameSink::OnUserCallCard(WORD wChairID, BYTE cbCallColor,BYTE cbBankScore)
{
	//效验状态
	...
	CallCard.cbBankScore=cbBankScore;
	...
	GamePlay.cbBankScore=cbBankScore;
	GamePlay.cbMainColor=m_cbMainColor;
...
bool CTableFrameSink::OnUserLandScore(WORD wChairID, BYTE bBossScore)
{
	...
	if(PassCount==4) 
	{
		OnEventGameEnd((wChairID+1)%m_wPlayerCount,NULL,GER_DISMISS);
		return true;
	}
	
	if (((m_bLandScore==5)||(PassCount==3))&&m_bLandScore!=250)
	{
		//设置变量
		if (m_bLandScore==0) m_bLandScore=5;
		
		if (m_wBankerUser==INVALID_CHAIR) m_wBankerUser=PassCountUser;
		
		//设置状态
		m_pITableFrame->SetGameStatus(GS_UG_BACK);

		if (PassCount==3) m_wBankerUser=PassCountUser;
		...
		SendConceal.wCurrentUser=m_wBankerUser;
		SendConceal.cbBankScore=m_bLandScore;
	...
	LandScore.bLandCount=m_bLandCount;
...
bool CTableFrameSink::OnUserOutCard(WORD wChairID, BYTE cbCardData[], BYTE cbCardCount)
{
	...
	{
		//类型判断
	    cbCardType=m_GameLogic.GetCardType(cbCardData,cbCardCount);
		//花色判断
		//if(cbCardCount==1) cbCardType=m_GameLogic.GetCardLogicColor(cbCardData,cbCardCount);
	...
	else
	{
		...
		//cbCardType=m_GameLogic.GetCardType(cbCardData,cbCardCount);  //出牌类型
		//花色判断
		//if(cbCardCount==1) cbCardType=m_GameLogic.GetCardLogicColor(cbCardData,cbCardCount);
	...
	if (m_wCurrentUser==INVALID_CHAIR)
	{
		...
		TurnBalance.cbBankScore=m_bLandScore;
		TurnBalance.cbMainColor=m_cbMainColor;
		TurnBalance.cbBankUser=m_wBankerUser;
...
bool CTableFrameSink::DispatchUserCard()
{
	...
	if((m_wFirstUser!=m_wBankerUser)&&(m_wBankerUser!=INVALID_CHAIR)) 
	{
		SendCard.wCurrentUser=m_wBankerUser;
	}
	else
	{
...
bool CTableFrameSink::OnUserSurrender(WORD cbBankerUser)
{
	//if (m_wBankerUser!=cbBankerUser) return false;
	m_bIsTX=true;
	OnEventGameEnd(cbBankerUser,NULL,GER_NORMAL);
	return true;
}
//////////////////////////////////////////////////////////////////////////
