...
const BYTE	CGameLogic::m_cbCardData[CELL_PACK]=
{
	...
	0x11,0x12,0x15,0x16,0x17,0x18,0x19,0x1A,0x1B,0x1C,0x1D,	//梅花 A - K
	0x21,0x22,0x25,0x26,0x27,0x28,0x29,0x2A,0x2B,0x2C,0x2D,	//红桃 A - K
	0x31,0x32,0x35,0x36,0x37,0x38,0x39,0x3A,0x3B,0x3C,0x3D,	//黑桃 A - K
	0x4E,0x4F,
};
...
BYTE CGameLogic::GetCardType(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	bool bMainColor=true;
	for (BYTE i=1;i<cbCardCount;i++) 
	{
		if (GetCardLogicColor(cbCardData[i])!=COLOR_NT) bMainColor=false;
	}
	...
	if (bMainColor==true) return CT_THROW_CARD;
	return CT_ERROR;
}
...
bool CGameLogic::EfficacyThrowCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, WORD wOutCardIndex, const BYTE cbHandCardData[GAME_PLAYER][MAX_COUNT], BYTE cbHandCardCount, tagOutCardResult & OutCardResult)
{
	...
	for (WORD i=0;i<GAME_PLAYER-1;i++)
	{
		...
		for (BYTE j=2;j>=2;j--)
		{
				...
				for (BYTE cbUserRound=0;cbUserRound<TractorDataInfo.cbTractorCount;cbUserRound++)
				{
					...
					for (BYTE cbOutRound=0;cbOutRound<pTractorDataInfo->cbTractorCount;cbOutRound++)
					{
						...
						if (cbLineCountUser>=cbLineCountOut)
						{
							...
							//对比大小
							if (CompareCardData(cbNextCardData,cbFisrtCardData)==true)
							{
			...
			if (pSameDataInfo->cbBlockCount>0)
			{
				...
				if (SameDataInfo.cbBlockCount>0)
				{
					...
					//对比大小
					if (CompareCardData(SameDataInfo.cbSameData[0],cbFisrtCardData)==true)
					{
		...
		if (AnalyseResult.SameDataInfo[0].cbCardCount!=0)
		{
			...
			//对比扑克
			if (CompareCardData(cbDistillCardHand[0],cbLessCardData)==true)
			{
...
//对比扑克
bool CGameLogic::CompareCardData(BYTE cbNextCardData, BYTE cbFirstCardData)
{
	//获取花色
	BYTE cbLogicColorNext=GetCardLogicColor(cbNextCardData);
	BYTE cbLogicColorFirst=GetCardLogicColor(cbFirstCardData);
	
	if(cbFirstCardData==0x4E&&cbNextCardData==0x4E) return false;  //比较小鬼
	if(cbFirstCardData==0x4F&&cbNextCardData==0x4F) return false;  //比较大鬼

	//比较花色
	if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData==cbNextCardData)) return false;   //同花色，都是例：梅花6
	

	////比较常主 2
	if (((GetCardValue(cbFirstCardData)==2)&&(GetCardValue(cbNextCardData)==2))) 
	{
		//比较花色
		if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData==cbNextCardData)&&(cbLogicColorNext==m_cbMainColor)&&(cbLogicColorFirst==m_cbMainColor))  //同花色，都是正2
		{
			return false;
		}

		if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData==cbNextCardData))  //同花色，都是负2
		{
			return false;
		}


		if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData!=cbNextCardData))  //同是常主，梅花2和方快2  非主2
		{
			if((GetCardColor(cbFirstCardData)==m_cbMainColor)&&(GetCardColor(cbNextCardData)!=m_cbMainColor))
			{
				return false;
			}
			if((GetCardColor(cbFirstCardData)!=m_cbMainColor)&&(GetCardColor(cbNextCardData)==m_cbMainColor))
	...
	////比较常主 7
	if (((GetCardValue(cbFirstCardData)==7)&&(GetCardValue(cbNextCardData)==7))) 
	{
		//比较花色
		if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData==cbNextCardData)&&(cbLogicColorNext==m_cbMainColor)&&(cbLogicColorFirst==m_cbMainColor))  //同花色，都是正2
		{
		...
		if((cbLogicColorNext==cbLogicColorFirst)&&(cbFirstCardData!=cbNextCardData))    //花色相同，值不同 
		{
			if((GetCardColor(cbFirstCardData)==m_cbMainColor)&&(GetCardColor(cbNextCardData)!=m_cbMainColor))
			{
				return false;
	...
	//对比大小
	if ((cbLogicColorNext==COLOR_NT)||(cbLogicColorNext==cbLogicColorFirst)) //比较常主或同花色
	{
		//获取大小
...
bool CGameLogic::CompareCardResult(const tagAnalyseResult WinnerResult, const tagAnalyseResult UserResult)
{
	...
	for (BYTE i=2;i>=2;i--)
	{
		...
		if ((pTractorDataInfoWinner->cbTractorCount>0)&&(pTractorDataInfoUser->cbTractorCount>0))
		{
			...
			//对比大小
			if (CompareCardData(cbNextCardData,cbFisrtCardData)==true) return true;
		}
	}
...
WORD CGameLogic::CompareCardArray(const BYTE cbOutCardData[GAME_PLAYER][MAX_COUNT], BYTE cbCardCount, WORD wFirstIndex)
{
	...
	case CT_TRACKOR_4:	//拖拉机型
		{
			//循环比较
			for(WORD i=GAME_PLAYER-1;i>=0&&i<GAME_PLAYER;i--)
			{
				//用户索引
				WORD wUserIndex=(wFirstIndex+i)%GAME_PLAYER;

				//牌型判断
				if (GetCardType(cbOutCardData[wUserIndex],cbCardCount)!=cbCardType) continue;

				//对比大小
				if (CompareCardData(cbOutCardData[wUserIndex][0],cbOutCardData[wWinnerIndex][0])==true) 
				{
...
bool CGameLogic::EfficacyOutCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, const BYTE cbFirstCardData[], BYTE cbFirstCardCount, const BYTE cbHandCardData[], BYTE cbHandCardCount)
{
	...
	if ((cbFirstCardType==CT_THROW_CARD)&&(cbOutCardType==0&&cbFirstCardType==8)&&(cbOutLogicColor==0xff)&&(cbFirstLogicColor==64)) return true;
	
	if ((cbFirstCardType==CT_TRACKOR_2)&&(cbOutCardType==0&&cbFirstCardType==5)&&(cbOutLogicColor==0xff)&&(cbFirstLogicColor==64)) return true;
...
BYTE CGameLogic::GetCardLogicValue(BYTE cbCardData)
{
	...
	if (cbCardValue==1) cbLogicValue+=13;  //A

	//主牌变换
	if (cbCardValue==m_cbMainValue)        //m_cbMainValue=7 else 2
	{
...
//逻辑花色2
BYTE CGameLogic::GetCardLogicColor2(BYTE cbCardData)
{
	//逻辑数值
	BYTE cbCardColor=GetCardColor(cbCardData);
	BYTE cbCardValue=GetCardValue(cbCardData);

	if(cbCardData==0x4E||cbCardData==0x4F) return COLOR_NT;  //大小鬼
	//if(cbCardData==0x4E||cbCardData==0x4F) MessageBox(0,0,0,0);  //大小鬼
