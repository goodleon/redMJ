...
#define WAV_START						3				//开始
#define WAV_WIN							5				//胜利
#define	WAV_LOST						6			    //输	
#define	WAV_OTHER						1005			//其他
#define WAV_dizhu						1006			//地主
#define WAV_double						1007			//双顺
#define	WAV_dan							1008			//单顺
#define	WAV_SDY							1014			//3 1
#define	WAV_SDE							1015			//3 2
#define WAV_friendsend					1010			//队友出牌
#define WAV_Enemy						1011			//对手出牌
#define WAV_pass						1012			//不要
#define WAV_gameover					1013			//结束
	...
	case GS_WK_FREE:	//空闲状态
		{
			...
			if (IsLookonMode()==false)
			{
				...
				SetWavScript("/GameApp/WavScript.asp?KindName=斗地主");
	...
	case GS_WK_SCORE:	//叫分状态
		{
			...
			//设置时间
			SetGameTimer(pStatusScore->wCurrentUser,IDI_LAND_SCORE,15);
	...
	case GS_WK_PLAYING:		//游戏状态
		{
			...
			//设置定时器
			SetGameTimer(pStatusPlay->wCurrentUser,IDI_OUT_CARD,20);
...
bool CGameClientDlg::OnSubSendCard(const void * pBuffer, WORD wDataSize)
{
	...
	//播放声音
	//PlayGameSoundFromFip2(WAV_START,GetMeChairID());
	PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_START"));

	//设置时间
	SetGameTimer(pSendCard->wCurrentUser,IDI_LAND_SCORE,15);
...
bool CGameClientDlg::OnSubLandScore(const void * pBuffer, WORD wDataSize)
{
	...
	//播放声音
	if(pLandScore->bLandScore!=255) //PlayGameSoundFromFip2(WAV_OTHER,GetMeChairID());
...
bool CGameClientDlg::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	if ((IsLookonMode()==false)&&(pGameStart->wCurrentUser==GetMeChairID()))
	{
		...
		//播放声音
		//PlayGameSoundFromFip2(WAV_dizhu,INVALID_CHAIR);
		PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_START"));
	}
...
/*		if(m_bTurnOutType==CT_DOUBLE_LINE) PlayGameSoundFromFip2(WAV_double,INVALID_CHAIR);
		if(m_bTurnOutType==CT_ONE_LINE) PlayGameSoundFromFip2(WAV_dan,INVALID_CHAIR);
		if(m_bTurnOutType==CT_THREE_LINE_TAKE_ONE) PlayGameSoundFromFip2(WAV_SDY,INVALID_CHAIR);
		if(m_bTurnOutType==CT_THREE_LINE_TAKE_DOUBLE) PlayGameSoundFromFip2(WAV_SDE,INVALID_CHAIR);
	*/	
		//设置定时器
	...
	if ((IsLookonMode()==false)&&(pOutCard->wCurrentUser==GetMeChairID()))
	{
		...
		for (BYTE n=0;n<3;n++)
		{
			if(n!=m_wLandUser&&n!=GetMeChairID())
			{
				//PlayGameSoundFromFip2(WAV_friendsend,n);
			}
		}
	}
	...
	if (pOutCard->wCurrentUser!=INVALID_CHAIR)
	{
		...
		SetGameTimer(pOutCard->wCurrentUser,IDI_OUT_CARD,(bCardCount<m_bTurnCardCount)?3:20);
	}
...
bool CGameClientDlg::OnSubPassCard(const void * pBuffer, WORD wDataSize)
{
	...
	{		
		...
		m_GameClientView.SetPassFlag(wViewChairID,true);
		//播放声音
		//PlayGameSoundFromFip2(WAV_pass,INVALID_CHAIR);
		m_GameClientView.m_UserCardControl[wViewChairID].SetCardData(NULL,0);
	}
	...
	if (m_bTurnCardCount!=0)
	{
		...
		SetGameTimer(pPassCard->wCurrentUser,IDI_OUT_CARD,(bCardCount<m_bTurnCardCount)?3:20);
	}
	else SetGameTimer(pPassCard->wCurrentUser,IDI_OUT_CARD,20);
...
bool CGameClientDlg::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	m_GameClientView.m_ScoreView.SetGameTax(pGameEnd->lGameTax,pGameEnd->IsBankUser);
	...
	//else PlayGameSoundFromFip2(WAV_gameover,GetMeChairID());
...
void CGameClientDlg::OnTimer(UINT nIDEvent)
{
	if ((nIDEvent==IDI_MOST_CARD)&&(m_wMostUser!=INVALID_CHAIR))
	{
		...
		//设置时间
		SetGameTimer(wCurrentUser,IDI_OUT_CARD,20);
