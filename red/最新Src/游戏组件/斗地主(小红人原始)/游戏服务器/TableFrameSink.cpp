	...
	case GER_NORMAL:		//常规结束
		{
			...
			if (m_pGameServiceOption->wServerType&GAME_GENRE_GOLD)
			{
				for (WORD i=0;i<m_wPlayerCount;i++)
				{
					//if (GameEnd.lGameScore[i]>=10L)
					//{
						GameEnd.lGameTax=GameEnd.lGameScore[i]*5/100;
			...
			if (k!=0) GameEnd.lGameTax=k*GameEnd.lGameTax;
...
bool CTableFrameSink::OnUserLandScore(WORD wChairID, BYTE bLandScore)
{
	...
	{
		OnEventGameEnd(INVALID_CHAIR,NULL,GER_NO_SCORE);
	}
	return true;
}
