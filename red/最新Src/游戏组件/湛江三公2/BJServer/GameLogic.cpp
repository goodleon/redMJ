...
{
	BYTE bValue = bCard%13;
	if (bValue == 11 || bValue == 12 || bValue == 13) bValue=0;
	return bValue;
}
...
BYTE CGameLogic::CompareSanCard(const BYTE bFirstList[], const BYTE bLastList[], BYTE bCardCount)
{
	...
	bLastCard = (GetCardValoe(bLastList[0])+GetCardValoe(bLastList[1])+GetCardValoe(bLastList[3]))%10;
							 ...
							 const BYTE bIdleCard[],  const BYTE bIdleCardCount)
{
	...
	case SHAN: return CompareSanCard(bBankerCard,bIdleCard,3);
		//是否为公牌
	case SanGong: return 1; //都为JQK庄赢
		//是否对子公牌
	case SanGongDui: return CompareSanGongDui(bBankerCard,bIdleCard);
		//是否三张一样公牌
	case SanZhangGong: return CompareOnlyOne(bBankerCard[0],bIdleCard[0]);
		//是否333超级三公
	case SanZhangSan: return 1; //这种情况1副牌不可能发生，除非是多副牌，预留 庄赢
	}
