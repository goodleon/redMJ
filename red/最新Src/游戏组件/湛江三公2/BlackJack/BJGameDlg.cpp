...
bool CBJGameDlg::OnSubInitCard(const void * pBuffer, WORD wDataSize)
{
	...
	for (WORD i = 0; i < GAME_PLAYER; i++)
	{
		...
		{					
			...
			cbCardData[0] = (wViewChairID == 2) ? pInitCard->cbFundusCard : pInitCard->cbFundusCard;
...
{		
	...
	//m_lCardNum = m_lCardNum + 1;
	//CString a;
	//a.Format("%d要牌第%d次",wViewChairID,m_lCardNum);
	////MessageBox(a);
	//if(m_lCardNum==3)
	//{
	//	m_GameView.OnStopCard();
