...
CBJGameView::CBJGameView(void) : CGameFrameView(true,24)
{
	...
	memset(m_cbCardType,0,sizeof(m_cbCardType));
...
void CBJGameView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (WORD i = 0; i < GAME_PLAYER; i++)
	{
		...
		if (pUserData != NULL)
		{
			if(m_cbCardType[i]==SHAN)
			{
				if(m_cbTopCardSum[i] !=0)
				{
					_snprintf(szBuffer, sizeof(szBuffer), TEXT("%d"), m_cbTopCardSum[i]%10);
			...
			{
				switch(m_cbCardType[i])
				{
				case SAN_GONG:
					{
						sprintf(szBuffer, TEXT("三公牌型"));
