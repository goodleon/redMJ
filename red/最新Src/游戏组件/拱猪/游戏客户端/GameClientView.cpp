...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	if (m_bDeasilOrder==true)
	{
		...
		m_ptName[1].y=nHeight/2-m_nYFace+3-60;
		...
		m_ptName[3].y=nHeight/2-m_nYFace+3-60;
	...
	m_ptName[2].y=nHeight-m_nYBorder-m_nYFace-102;
	m_ptTimer[2].x=nWidth/2-m_nXFace/2-m_nXTimer*2-5;
	m_ptTimer[2].y=nHeight-m_nYBorder-m_nYTimer-110;
...
//	DeferWindowPos(hDwp,m_btStart,NULL,(nWidth-rcButton.Width())/2,nHeight-275-m_nYBorder,0,0,uFlags);
	...
	DeferWindowPos(hDwp,m_btOutCard,NULL,(nWidth-rcButton.Width())/2,nHeight-300-m_nYBorder+10,0,0,uFlags);
	DeferWindowPos(hDwp,m_btShowCard,NULL,(nWidth)/2-rcButton.Width(),nHeight-300-m_nYBorder,0,0,uFlags);
	DeferWindowPos(hDwp,m_btNotShowCard,NULL,(nWidth)/2,nHeight-300-m_nYBorder,0,0,uFlags);
	...
	m_HandCardControl.SetBenchmarkPos(nWidth/2 , nHeight-210-24 , enXCenter , enYCenter);
	...
	if (m_bDeasilOrder==true)
	{
		...
		m_UserCardControl[2].SetBenchmarkPos(nWidth/2 , nHeight-307 , enXCenter , enYBottom);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	m_ImageScoreLeft.BitBlt(pDC->m_hDC , nPosX , nPosY , SRCCOPY);
	...
	nPosY		 = rect.bottom-nImageHeight-bRise ;
	m_ImageScoreMid.BitBlt(pDC->m_hDC , nPosX , nPosY , SRCCOPY) ;
	m_ImageScoreMid.BitBlt(pDC->m_hDC , nPosX+nImageWidth , nPosY , SRCCOPY) ;
	...
	nPosY		 = rect.bottom-nImageHeight-bRise ;
	m_ImageScoreSeparator.BitBlt(pDC->m_hDC , nPosX , nPosY , SRCCOPY);
	...
	nPosY		 = rect.bottom-nImageHeight -bRise;
	m_ImageScoreRight.BitBlt(pDC->m_hDC , nPosX , nPosY , SRCCOPY);

	//显示文字
	pDC->SetTextColor(RGB(255,255,255)) ;
	UINT uOldAlign = pDC->SetTextAlign(TA_LEFT|TA_TOP);
	pDC->TextOut(37 ,  rect.bottom-nImageHeight+4-bRise , "用户名" , lstrlen("用户名")) ;
    pDC->TextOut(158 , rect.bottom-nImageHeight+4 -bRise, "亮牌" ,  lstrlen("亮牌")) ;
    
    CRect DrawRect ;
	DrawRect.left	= m_ImageScoreLeft.GetWidth()+5+90+m_ImageScoreSeparator.GetWidth()+120+6+10;
	DrawRect.top	= rect.bottom - m_ImageScoreSeparator.GetHeight()+4-bRise ;
	DrawRect.bottom = rect.bottom - 77 ;
	...
	if(true==m_bShowCardHint) 
	{
		...
		DrawRect.top	= 20-bRise;
		DrawRect.left	= 20 ;
		DrawRect.bottom	= nHeight - 20 ;
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pUserData!=NULL)
		{
			...
			pDC->TextOut(50,nHeight-21-(3-i)*18-bRise,pUserData->szName,lstrlen(pUserData->szName));
