...
BOOL CDlgToolBar::OnInitDialog()
{
	...
	m_btChatSend->Create(NULL,WS_CHILD|WS_VISIBLE,CRect(157,3,0,0),this,IDOK);
	...
	m_ChatEdit->Create(WS_CHILD|WS_VISIBLE|ES_AUTOHSCROLL|ES_NOHIDESEL|WS_BORDER, CRect(10,7,148,22),this,0);
...
void CDlgToolBar::OnOK()
{
	...
	if (szChatMessage[0]!=0)
	{
		...
		//m_pICK->SendChatPacket(dwTargetUserID,szChatMessage,g_GlobalOption.m_crChatTX);
...
BOOL CDlgChatMsg::OnInitDialog()
{
	...
	//m_ChatMsg.Create(WS_CHILD|SS_NOTIFY|WS_VISIBLE|ES_MULTILINE | ES_NOHIDESEL | ES_READONLY | ES_WANTRETURN | WS_VSCROLL,
	//	CRect(7,6,224,66),this,IDC_CHAT_MESSAGE);
	m_ChatMsg.Create(WS_CHILD|SS_NOTIFY|WS_VISIBLE|ES_MULTILINE | ES_NOHIDESEL | ES_READONLY | ES_WANTRETURN | WS_VSCROLL,
		CRect(2,2,178,75),this,IDC_CHAT_MESSAGE);
	//m_ChatMsg.LimitText(50);
	SetWindowPos(NULL,0,0,299,159,SWP_NOMOVE);
	m_ChatMsg.SetBackgroundColor(FALSE,RGB(222,219,219));
	//m_ChatMsg.SetBackgroundColor(FALSE,RGB(41,52,67));
