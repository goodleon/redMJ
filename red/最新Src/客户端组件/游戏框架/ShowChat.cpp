...
{
	//AfxMessageBox("1");//123456789012345678
int nIndex=0;
	...
	for(WORD i=0;i<MAX_PLAYER;i++)
	{
		if(m_lTime[i]>0)
		{
			...
			else
			{
				...
				if(pMsgImage)
				{
					CImageHandle Msg_THandle(pMsgImage);
					//pMsgImage->BitBlt(pDC->GetSafeHdc(),x,y);
					pMsgImage->AlphaDrawImage(pDC,x, y, pMsgImage->GetWidth(), pMsgImage->GetHeight(),0,0,RGB(255,0,255));
					
					//AlphaDrawImage(pDC,x, y, pMsgImage->GetWidth(), pMsgImage->GetHeight(),0,0,RGB(255,0,255));
