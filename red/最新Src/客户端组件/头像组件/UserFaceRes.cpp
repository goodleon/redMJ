...
CUserFaceRes::CUserFaceRes(void)
{
	...
	m_SmallFace.LoadFromResource(hInstancle,IDB_FACE_SMALL);
	m_BigFace.LoadFromResource(hInstancle,IDB_FACE_BIG);
	...
	m_BigSize.SetSize(BIG_FACE_WIDTH,BIG_FACE_HEIGHT);
...
//绘画头像
void __cdecl CUserFaceRes::DrawBigFace(CDC * pDC, int nXPos, int nYPos, WORD wFaceID)
{
	//调整参数
	if (wFaceID>=m_wCount) wFaceID=0;
	...
	CDC DCFace;
	DCFace.CreateCompatibleDC(NULL);
	DCFace.SelectObject(m_BigFace);
	AlphaDrawImage(pDC,nXPos,nYPos,m_BigSize.cx,m_BigSize.cy,&DCFace,wFaceID*m_BigSize.cx,0);
