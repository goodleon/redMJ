...
bool __cdecl CClientKernel::InitClientKernel(LPCTSTR lpszComLine, IUnknownEx * pIUnknownEx)
{
	...
	if (lpszComLine[0]!=0)
	{
		...
		GetCommandParam(TEXT(" /RoomToken:"),strRoomToken);
...
//设置用户状态
void __cdecl CClientKernel::SetUserFlags(BYTE bStatus)
{
	if (m_pMeUserItem==NULL) return;
	m_pMeUserItem->cbUserStatus=bStatus;
}
