...
BOOL CDlgOptionSystem::OnInitDialog()
{
	...
	//启用背景音乐
	((CButton *)GetDlgItem(IDC_BGSOUND))->SetCheck(g_GlobalOption.m_bDisableBgSound);
	return TRUE;
}
...
bool CDlgOptionTableRule::SaveOptions()
{
	...
	{
		AfxMessageBox(TEXT("游戏币限制范围参数不正确，请正确设置游戏币限制范围！"),MB_ICONINFORMATION);
...
BOOL CDlgOption::OnInitDialog()
{
	...
	//创建控件
	m_ButtonBar.Create(NULL,NULL,WS_CHILD,CRect(-999,0,0,0),this,10);
...
{
	//保存参数
	g_GlobalOption.m_bDisableBgSound=(((CButton *)m_SystemItem.GetDlgItem(IDC_BGSOUND))->GetCheck()==BST_CHECKED)?TRUE:FALSE;
	AfxGetApp()->WriteProfileInt(REG_GLOBAL_OPTIONS,TEXT("DisableBgSound"),g_GlobalOption.m_bDisableBgSound);
	g_GlobalOption.CloseMedia();//停止所有音乐
