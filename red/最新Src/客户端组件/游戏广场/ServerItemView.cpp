...
bool CServerListManager::InsertKindItem(tagGameKind GameKind[], WORD wCount)
{
	...
	try
	{
		...
		//	MessageBox(0,GameKind[i].szProcessName,0,0);
...
//bluker:是否有该类型服务器在运行
bool CServerListManager::IsExistKindServerRunning(WORD wKindID)
{
	for (INT_PTR i=0;i<m_PtrArrayServer.GetCount();i++)
	{
		CListServer * pListServer=m_PtrArrayServer[i];
		tagGameServer * pGameServer=pListServer->GetItemInfo();
		if(!pGameServer)break;
		if (pGameServer->wKindID==wKindID) return true;
	}
	return false;
}
	...
	case ItemGenre_Server:		//游戏房间
		{
			...
			//bluker:已安装的游戏
			//CString s;
			//s.Format("KindId=%d ServerId=%d %s",pGameServer->wKindID,pGameServer->wServerID,szItemTitle);
			//MessageBox(s);
