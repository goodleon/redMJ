...
//加载资源
bool CTableResource::LoadFromResource(HINSTANCE hInstance,WORD wChairCount)
{
	//效验参数
	...
	TCHAR ResGame_Table_Lock[32];
	TCHAR ResGame_Table_Ready[32];
	TCHAR ResGame_Table_Play[32];
	TCHAR ResGame_Table_Normal[32];
	TCHAR ResLocation_info[32];

	sprintf(ResGame_Table_Lock,TEXT("GAME_TABLE_LOCK%d"),wChairCount);
	sprintf(ResGame_Table_Ready,TEXT("GAME_TABLE_READY%d"),wChairCount);
	sprintf(ResGame_Table_Play,TEXT("GAME_TABLE_PLAY%d"),wChairCount);
	sprintf(ResGame_Table_Normal,TEXT("GAME_TABLE_NORMAL%d"),wChairCount);
	sprintf(ResLocation_info,TEXT("LOCATION_INFO%d"),wChairCount);

	try
	{
		//加载锁位图
		m_ImageLock.LoadFromResource(hInstance,ResGame_Table_Lock);
		if (m_ImageLock.IsNull())throw 0;

		//加载准备标志
		m_ImageReady.LoadFromResource(hInstance,ResGame_Table_Ready);
		if (m_ImageReady.IsNull())throw 0;

		//加载开始标志
		m_ImagePlay.LoadFromResource(hInstance,ResGame_Table_Play);
		if (m_ImagePlay.IsNull()) throw 0;

		//加载桌子位图
		m_ImageTable.LoadFromResource(hInstance,ResGame_Table_Normal);
		...
		//加载配置RT_RCDATA
		hResour=FindResource(hInstance,ResLocation_info,TEXT("BIN"));
...
bool __cdecl CTableFrame::InitTableFrame(WORD wTableCount, WORD wChairCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx)
{
	...
	if(!m_TableResource.LoadFromResource(AfxGetInstanceHandle(),wChairCount))
	{
...
{
	if ((m_bSitByServer==false)&&(m_bRightMouseDown==false)&&(m_bLeftMouseDown==false))
	{
		//变量定义
		int iTableWidth=m_TableResource.m_wWidth;
		int iTableHeight=m_TableResource.m_wHeight;
