...
void CServiceParameter::OnOK()
{
	...
	//总下注上限
	OptionParameter.GameServiceOption.lMaxGold=GetDlgItemInt(IDC_MAX_GOLD,NULL,FALSE);

	//限制局数
	OptionParameter.GameServiceOption.cbLimitDraw=(((CButton *)GetDlgItem(IDC_LIMIT_DRAW))->GetCheck()==BST_CHECKED)?TRUE:FALSE;

	//bluker:只能由服务器指定座位
	OptionParameter.GameServiceOption.wSitByServer=(((CButton *)GetDlgItem(IDC_SIT_BYSERVER))->GetCheck()==BST_CHECKED)?TRUE:FALSE;
...
void CServiceParameter::UpdateOptionParameter()
{
	...
	//总下注上限
	if (m_OptionParameter.GameServiceOption.lMaxGold==0) SetDlgItemText(IDC_MAX_GOLD,TEXT(""));
	else SetDlgItemInt(IDC_MAX_GOLD,m_OptionParameter.GameServiceOption.lMaxGold,FALSE);
	...
	//bluker:只能由服务器指定座位
	WORD wSitByServer=m_OptionParameter.GameServiceOption.wSitByServer;
	((CButton *)GetDlgItem(IDC_SIT_BYSERVER))->SetCheck((wSitByServer==TRUE)?BST_CHECKED:BST_UNCHECKED);
