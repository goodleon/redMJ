...
#define MY_WM_GAMESERVER_LOAD	(WM_USER+5)		// 中心服务器系统托盘消息
...
BOOL CServiceLoaderDlg::OnInitDialog()
{
	...
	//加载配置
	//bluker:加[-start]和[-load]的参数
	CString szCmdLine=GetCommandLine();
	szCmdLine.MakeLower();
	int p;
	p=szCmdLine.Find(" -load ");
	if(p>0)
	{
		szCmdLine=szCmdLine.Mid(p+7);

		TCHAR szDescribe[128]=TEXT("");

		bool bSuccess=m_ServiceParameter.LoadParameter(szCmdLine);
		if (bSuccess==true)
		{
			//控制界面
			...
			//提示信息
			_snprintf(szDescribe,sizeof(szDescribe),TEXT("“%s”游戏房间加载成功"),szCmdLine.LockBuffer());
		...
		{
			_snprintf(szDescribe,sizeof(szDescribe),TEXT("“%s”游戏房间加载失败"),szCmdLine.LockBuffer());
			ShowErrorMessasge(szDescribe,Level_Exception);
		}
		szCmdLine.UnlockBuffer();
	}
...
void CServiceLoaderDlg::OnSize(UINT nType, int cx, int cy)
{
	CDialog::OnSize(nType, cx, cy);

	//---------------------------------------------------------------------------(+)
	HWND IGameWnd=::FindWindow(TEXT("#32770"),CenterServerTitle);
	if (IGameWnd!=NULL) 
	{
		::PostMessage(IGameWnd,MY_WM_GAMESERVER_LOAD,0,(long)this->GetSafeHwnd());
