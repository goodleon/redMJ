...
bool CServerListInfo::ResetInfo()
{
	...
	//------------加有奖游戏---------------------------------------(+)
	TCHAR szGameListFile[MAX_PATH];
	GetModuleFileName(0,szGameListFile,MAX_PATH);
	*_tcsrchr(szGameListFile,'.') = 0;
	_tcscat(szGameListFile,".ini");
	LPTSTR pGames = new TCHAR[65536];

	if(!pGames)return true;
	DWORD iSecLen=GetPrivateProfileSection("有奖游戏", pGames, 65536-1, szGameListFile);
	if(iSecLen<1)return true;

	tagGameServer GameServer;
	memset(&GameServer,0,sizeof(GameServer));
	GameServer.wKindID = PRIZE_GAME_ID;

	int iBeginId = 1000;
	DWORD i=0;
	while(pGames[i]!=0 && i<iSecLen)
	{
		GameServer.wServerID = iBeginId++;
		_snprintf(GameServer.szServerName,sizeof(GameServer.szServerName),"%s",pGames+i);
		GameServer.szServerName[sizeof(GameServer.szServerName)-1] = 0;
		InsertGameServer(&GameServer);
		i += (DWORD)_tcslen(pGames+i)+1;
	}
	delete []pGames;
	//------------加有奖游戏---------------------------------------(-)
...
bool __cdecl CServerList::SendGameKindList(WORD wIndex, WORD wRountID)
{
	...
	if (wSendSize>0)
	{
		m_pITCPSocketEngine->SendData(wIndex,wRountID,MDM_GP_SERVER_LIST,SUB_GP_LIST_KIND,m_cbSendBuffer,wSendSize);
	////////			/*char a[32];
	////////sprintf(a,"%s----%d",pGameKind->szKindName,wRountID );
	////////AfxMessageBox(a);*/
	}
	return true;
}
