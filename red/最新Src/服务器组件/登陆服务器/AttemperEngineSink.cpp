...
bool CAttemperEngineSink::OnDBLogonSuccess(void * pDataBuffer, WORD wDataSize, NTY_DataBaseEvent * pDataBaseEvent)
{
	...
	SendPacket.AddPacket(pDBRLogonSuccess->szSessionId,CountString(pDBRLogonSuccess->szSessionId),DTP_SESSIONID);
	...
	m_ServerList.SendGameKindList(wIndex,wRoundID);//bluker:,在GameKindInfo加"有奖游戏"
	m_ServerList.SendGameStationList(wIndex,wRoundID);
	m_ServerList.SendGameServerList(wIndex,wRoundID);//bluker:加"有奖游戏列表"
	...
	case SUB_GP_LOGON_ACCOUNTS:		//帐号登录
		{
			...
			//CString a;
			//a.Format("传过来:%x,自己的%x",pLogonByAccounts->dwPlazaVersion,GetPlazaRoomVersion());
			//AfxMessageBox(a);
			////////////////效验版本       李明改
			//////////////if (pLogonByAccounts->dwPlazaVersion!=::GetPlazaRoomVersion())
			//////////////{
			//////////////	//获取版本
			//////////////	WORD wHigh=LOWORD(pLogonByAccounts->dwPlazaVersion);
	...
	case SUB_GP_LOGON_USERID:		//ID 登录
		{
			...
			//效验版本
			if (pLogonByUserID->dwPlazaVersion!=::GetPlazaRoomVersion())
			{
				//获取版本
				WORD wHigh=LOWORD(pLogonByUserID->dwPlazaVersion);
	...
	case SUB_GP_REGISTER_ACCOUNTS:		//注册帐号
		{
			...
			//效验版本
			if (pRegisterAccounts->dwPlazaVersion!=::GetPlazaRoomVersion())
			{
				//获取版本
				WORD wHigh=LOWORD(pRegisterAccounts->dwPlazaVersion);
