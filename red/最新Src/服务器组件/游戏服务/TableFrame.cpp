...
bool __cdecl CTableFrame::PerformLookonAction(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	{
		//发送消息
		LPCTSTR pszMessage=TEXT("您不能旁观，若需要帮助，请联系客服！");
	...
	{
		LPCTSTR pszDescribe=TEXT("游戏房间即将关闭，不允许再进入游戏桌！");
	...
	{
		LPCTSTR pszDescribe=TEXT("很抱歉，此游戏桌现在不允许普通玩家进入！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
	...
	{
		LPCTSTR pszDescribe=TEXT("玩家已离开，不能旁观！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
	...
	if (m_bLocked==true)
	{
		...
		{
			LPCTSTR pszDescribe=TEXT("密码错误，不能旁观！");
			SendSitFailedPacket(pIServerUserItem,pszDescribe);
			return false;
		}
	}
	...
	{
		LPCTSTR pszDescribe=TEXT("游戏还没有开始，不能旁观！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
...
bool __cdecl CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem)
{
	...
	{
		LPCTSTR pszDescribe=TEXT("由于此游戏房间即将关闭，不允许再进入游戏桌！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
	...
	{
		LPCTSTR pszDescribe=TEXT("抱歉，此游戏桌现在不允许普通玩家进入！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
	...
	if (m_pIUserItem[wChairID]!=NULL)
	{
		...
		_snprintf(szDescribe,sizeof(szDescribe),TEXT("位置已经被 [ %s ] 捷足先登了，请选择其他座位！"),pTableUserData->szAccounts);
	...
	if (m_bStarted==true)
	{
		LPCTSTR pszDescribe=TEXT("游戏已经开始了，不能进入游戏桌！");
		SendSitFailedPacket(pIServerUserItem,pszDescribe);
		return false;
	}
	...
	if (m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)
	{
		...
		{
			TCHAR szDescribe[128]=TEXT("");
			lstrcpyn(szDescribe,TEXT("您好，这是游戏比赛房间，你没有比赛权限，所以不能坐到此位置上！ "),CountArray(szDescribe));
			SendSitFailedPacket(pIServerUserItem,szDescribe);
			return false;
		}
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if ((m_pIUserItem[i]!=NULL)&&(m_pIUserItem[i]->GetClientIP()==dwUserIP))
			{
				...
				{
					LPCTSTR pszDescribe=TEXT("比赛不允许相同 IP 地址的玩家游戏，此游戏桌存在与你 IP 地址相同的玩家，不能加入游戏！");
					SendSitFailedPacket(pIServerUserItem,pszDescribe);
					return false;
				}
				else
		...
		for (WORD i=0;i<m_wChairCount-1;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				for (WORD j=i+1;j<m_wChairCount;j++)
				{
					...
					{
						LPCTSTR pszDescribe=TEXT("比赛不允许相同 IP 地址的玩家游戏，此游戏桌存在 IP 地址相同的玩家，不能加入游戏！");
						SendSitFailedPacket(pIServerUserItem,pszDescribe);
						return false;
					}
				}
		...
		if ((m_pGameServiceOption->cbLimitDraw==TRUE)&&(m_pGameServiceOption->lMatchDraw>0))
		{
			...
			LONG lPlayCount=pUserData->UserScoreInfo.lLostCount; //比赛新规则
	...
	else
	{
		...
		if (m_pGameServiceOption->lLessScore!=0L)
		{
			...
			{
				TCHAR szDescribe[128]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),szLessMoney,m_pGameServiceOption->lLessScore);
				SendSitFailedPacket(pIServerUserItem,szDescribe);
				return false;
			}
		}
		...
		if (m_pGameServiceOption->lHighScore!=0L)
		{
			...
			{
				TCHAR szDescribe[128]=TEXT("");
				_snprintf(szDescribe,sizeof(szDescribe),TEXT("你的游戏币高于 %ld，不能加入游戏！"),m_pGameServiceOption->lHighScore);
				SendSitFailedPacket(pIServerUserItem,szDescribe);
				return false;
			}
		}*/
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				if (pTableUserRule->bLimitScore==true)
				{
					...
					{
						TCHAR szDescribe[128]=TEXT("");
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("你的游戏币太高，与 %s 设置的设置不符，不能加入游戏！"),m_pIUserItem[i]->GetAccounts());
						SendSitFailedPacket(pIServerUserItem,szDescribe);
						return false;
					}
					...
					{
						TCHAR szDescribe[128]=TEXT("");
						_snprintf(szDescribe,sizeof(szDescribe),TEXT("你的游戏币太低，与 %s 设置的设置不符，不能加入游戏！"),m_pIUserItem[i]->GetAccounts());
						SendSitFailedPacket(pIServerUserItem,szDescribe);
						return false;
					}
				}
...
bool __cdecl CTableFrame::OnEventSocketFrame(WORD wSubCmdID, const void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	//	CString a;
	//	a.Format("wSubCmdID:%d,data:%s,size:%d",wSubCmdID,pUserChat->szChatMessage,wDataSize);
	//	AfxMessageBox(a);
		...
		case SUB_GF_USER_CHAT:	//用户聊天
			{
				...
				//状态脏话
				if (g_FilterString.IsBadString(pUserChat->szChatMessage))
				{
					LPCTSTR pszMessage=TEXT("请注意卫生，保护公共环境！");
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				if (lPlayCount>=m_pGameServiceOption->lMatchDraw)
				{
					...
					lstrcpyn(szDescribe,TEXT("您好，谢谢你的参与，比赛已经结束，请等待赛果公布！ "),CountArray(szDescribe));
	...
	if ((lLessScore!=0L)||(lHighScore!=0))
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				{
					TCHAR szDescribe[256]=TEXT("");
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("您的游戏币不够，必须 %ld 以上才能进行游戏，请及时充值！"),lLessScore);
					SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
					PerformStandUpAction(pIServerUserItem);
				}
				...
				{
					TCHAR szDescribe[256]=TEXT("");
					_snprintf(szDescribe,sizeof(szDescribe),TEXT("恭喜！您的游戏币已经多 %ld 以上，请到更高级的游戏房间！"),lHighScore);
					SendGameMessage(pIServerUserItem,szDescribe,SMT_EJECT|SMT_INFO|SMT_CLOSE_GAME);
					PerformStandUpAction(pIServerUserItem);
				}
			}
	...
	if ((m_pIGameServiceFrame->IsAllowEnterGame()==false)||(m_pIGameServiceFrame->IsShallClose()==true))
	{
		for (WORD i=0;i<m_wChairCount;i++)
		{
			if (m_pIUserItem[i]!=NULL)
			{
				...
				{
					//用户起立
					LPCTSTR pszMessage=TEXT("请注意，游戏房间即将关闭，请您暂时离开游戏！");
...
bool __cdecl CTableFrame::SendGameScene(IServerUserItem * pIServerUserItem, void * pData, WORD wDataSize)
{
	...
	if (m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)
	{
		_snprintf(szMessage,sizeof(szMessage),TEXT("您好，欢迎您参加“%s”比赛游戏！"),m_pGameServiceAttrib->szKindName);
		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
	else
	{
		_snprintf(szMessage,sizeof(szMessage),TEXT("您好，欢迎您来到金酷游戏中心！"),m_pGameServiceAttrib->szKindName);
		SendGameMessage(pIServerUserItem,szMessage,SMT_INFO);
	}
