...
bool __cdecl CAttemperEngineSink::StartService(IUnknownEx * pIUnknownEx)
{
	...
	{
		lstrcpyn(ColumnItem[2].szColumnName,TEXT("游戏币"),CountArray(ColumnItem[2].szColumnName));
	}
...
//bluker:服务器预留一个位置给玩家
bool __cdecl CAttemperEngineSink::ServerAllocTable(IServerUserItem * pIServerUserItem)
{
	//发送桌子信息
	...
	//找一个没有开始而且人最多的位置
	WORD CurTable=INVALID_TABLE;
	WORD CurChair=INVALID_CHAIR;
	SHORT nMaxCount= -1;
	for (WORD i=0;i<TableInfo.wTableCount;i++)
	{
		...
		{
			WORD nTmpCount=0;
			WORD nTmpChair=INVALID_CHAIR;

			for (WORD j = 0; j < pTableFrame->GetChairCount(); j++)
			{
				if ((pTableFrame->GetServerUserItem(j)!=NULL))
				{
					nTmpCount++;
				}
				else
				{
					if(nTmpChair==INVALID_CHAIR)
					{
						nTmpChair=j;
					}
				}
			...
			//CString s;
			//s.Format("%d %d %d %d",nMaxCount,nTmpCount,i,nTmpChair);
			//::MessageBox(0,s,0,0);

			if(nTmpChair != INVALID_CHAIR && (nMaxCount < 0 || nMaxCount<nTmpCount))
			{
				CurTable = i;
				CurChair = nTmpChair;
				nMaxCount = nTmpCount;
	...
	//如果找到合适的位置
	if(CurTable != INVALID_TABLE)
	{
		pTableFrame=(m_pTableFrame+CurTable);
		if(pTableFrame->PerformSitDownAction(CurChair,pIServerUserItem))return true;
	}


	CMD_GR_SitFailed SitFailed;
	_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe),szNoChairToSit);
	//发送数据
...
bool CAttemperEngineSink::OnDBLogonSuccess(void * pDataBuffer, WORD wDataSize, NTY_DataBaseEvent * pDataBaseEvent)
{
	...
	//发送消息
	LPCTSTR pszMessage=TEXT("欢迎进入"szProductName"");
...
bool __cdecl CAttemperEngineSink::OnEventCenterSocketConnect(int iErrorCode)
{
	...
	RegGameServer.GameServer.wSitByServer=m_pGameServiceOption->wSitByServer;
	RegGameServer.GameServer.lMaxGold=m_pGameServiceOption->lMaxGold;
	...
	case SUB_GR_USER_LOOKON_REQ:	//旁观桌子
		{
			...
			//bluker:只能由服务器指定座位
			if(m_pGameServiceOption->wSitByServer)
			{
				//构造数据
				CMD_GR_SitFailed SitFailed;
				memset(&SitFailed,0,sizeof(SitFailed));

				if (Command.wSubCmdID==SUB_GR_USER_LOOKON_REQ)
				{
					//_snprintf(SitFailed.szFailedDescribe,sizeof(SitFailed.szFailedDescribe),TEXT("抱歉，本房间不能旁观，谢谢合作！"));
					//发送数据
					//WORD wSendSize=sizeof(SitFailed)-sizeof(SitFailed.szFailedDescribe)+CountString(SitFailed.szFailedDescribe);
					//SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_SIT_FAILED,&SitFailed,wSendSize);
					SendData(pIServerUserItem,MDM_GR_USER,SUB_GR_SIT_FAILED);
					return true;
				}
				return ServerAllocTable(pIServerUserItem);
			};
