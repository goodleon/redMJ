...
extern "C" SKIN_CONTROL_CLASS DWORD SetWindowRgnFromRes(HWND hwnd,HINSTANCE hInst,LPCSTR pszResName,LPCSTR pszResType)
{
HRSRC           hRes;
HGLOBAL       hGlobal;
DWORD           dwSize;
RGNDATA*     pRgnData;
	...
	//Find   region   file   
	hRes         =   FindResource(hInst,pszResName,pszResType);   
	if(!hRes)return -1;
	...
	if(pRgnData){
		...
		::SetWindowRgn(hwnd,(HRGN)BackRgn.Detach(),TRUE);
		UnlockResource(pRgnData);
		dwRet=0;
	}
...
void SKIN_CONTROL_CLASS LayerWindow(HWND hwnd,int nTransparent)
{
