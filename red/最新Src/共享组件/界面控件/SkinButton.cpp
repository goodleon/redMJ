...
//宏定义
#define DEF_TEXT_COLOR				RGB(0,0,0)							//默认颜色
...
//设置标题位置
void CSkinButton::SetTitlePos(int x,int y)
{
	m_TitlePt.x=x;
	m_TitlePt.y=y;
}
...
void CSkinButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if(	m_TitlePt.x == (-999) &&	m_TitlePt.y == (-999))
		DrawText(hDC,strText,strText.GetLength(),ClientRect,DT_CENTER|DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
	else
	{
		ClientRect.left=m_TitlePt.x;
		ClientRect.top=m_TitlePt.y;
		DrawText(hDC,strText,strText.GetLength(),ClientRect,DT_SINGLELINE|DT_END_ELLIPSIS);
	}
...
//加载位图
bool CRgnButton::LoadRgnImage(HINSTANCE hInstance, UINT uBitmapID, COLORREF crTrans,HRGN hrgn)
{
	if(!m_ImageBack.SetLoadInfo(uBitmapID,hInstance))return false;
	return CreateControlRgn(crTrans,hrgn);
}

//加载位图
bool CRgnButton::LoadRgnImage(LPCTSTR pszFileName, COLORREF crTrans,HRGN hrgn)
{
	if(!m_ImageBack.SetLoadInfo(pszFileName))return false;
	return CreateControlRgn(crTrans,hrgn);
}

HRGN CRgnButton::MakeRegion(HDC hdc,LONG width,LONG height,COLORREF crTrans)
{
	COLORREF color; 
	int i,j; 
	HRGN rgn1,rgn2; 
...
//创建区域
bool CRgnButton::CreateControlRgn(COLORREF crTrans,HRGN hrgn)
{
	if(!m_ImageBack.IsSetLoadInfo())return false;
	CImageHandle ImageHandle(&m_ImageBack);
	if (!ImageHandle.IsResourceValid())return false;
	...
	if(hrgn==NULL)
	{
		HDC hdc=m_ImageBack.GetDC();
		hRgn=MakeRegion(hdc,nButtonWidth,nButtomHeight,crTrans);
...
void CRgnButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	else
	{
		ClientRect.left=m_TitlePt.x;
		ClientRect.top=m_TitlePt.y;
		DrawText(lpDrawItemStruct->hDC,strText,strText.GetLength(),ClientRect,DT_SINGLELINE|DT_END_ELLIPSIS);
	}
	return;
}
...
//设置标题位置
void CRgnButton::SetTitlePos(int x,int y)
{
	m_TitlePt.x=x;
	m_TitlePt.y=y;
}
